...
public class CommandAddStation extends Command {
	...
	/**
	 * CommandAddStation command.
	 * This command allows the administrator to add stations to the route
	 * @author t.Kosten
	 * 
	 */
	
	private static final long serialVersionUID = 132323L;
	
	private static final Logger LOG = Logger.getLogger(CommandAddStation.class);
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)
		...
		if (flag) {
			if (request.getParameter("add_station") != null) {
				String station = request.getParameter("add_station");
				DBManager.getInstance().addStationToRoute(id, min, hour, station);
				LOG.trace(station + " station added to route id " + id);
			} else {
				LOG.error("Incorrect input");
				if ("rus".equals(text.getLg())){
					error = "Не корректный ввод";
